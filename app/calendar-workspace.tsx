...
export default function CalendarWorkspace({ items, onUpdated, onNotice }: { items: Action[]; onUpdated: () => void; onNotice: (message: string) => void }) {
  const [selectedDay,setSelectedDay]=useState(() => { const now=new Date(); return dayStart(now.getHours()>=16?addDays(now,1):now); });
  const [events,setEvents]=useState<CalendarEvent[]>([]);
  const [receipt,setReceipt]=useState<CalendarReceipt>(null);
  const [busy,setBusy]=useState(false);

  const load=async()=>{ const start=dayStart(selectedDay); const end=addDays(start,1); const response=await fetch(`${runnerUrl}/api/calendar?start=${encodeURIComponent(start.toISOString())}&end=${encodeURIComponent(end.toISOString())}`,{cache:"no-store"}); const data=await response.json(); if(!response.ok) throw new Error(data.error||"Calendar could not load."); setEvents(data.events||[]); setReceipt(data.receipt||null); };
  useEffect(()=>{ let cancelled=false; const run=async()=>{ try { await load(); if(cancelled)return; await fetch(`${runnerUrl}/api/calendar/refresh`,{method:"POST",headers:{"content-type":"application/json"},body:"{}"}); window.setTimeout(()=>{if(!cancelled)void load();},1400); } catch(error){if(!cancelled)onNotice(error instanceof Error?error.message:"Calendar could not load.");} }; void run(); return()=>{cancelled=true;}; },[selectedDay]);

  const timed=events.filter(event=>!event.isAllDay).sort((a,b)=>a.startAt.localeCompare(b.startAt));
  const allDay=events.filter(event=>event.isAllDay);
  const dayActions=items.filter(item=>!["done","dismissed","queued","working"].includes(item.status) && item.decisionState!=="proposed" && (sameDay(item.plannedAt,selectedDay) || (!item.plannedAt && (!item.dueAt || new Date(item.dueAt)<=addDays(selectedDay,1)))));
  const planned=dayActions.filter(item=>sameDay(item.plannedAt,selectedDay)).sort((a,b)=>String(a.plannedAt).localeCompare(String(b.plannedAt)));
  const suggestions=useMemo(()=>{
    const start=new Date(selectedDay); start.setHours(8,30,0,0); const end=new Date(selectedDay); end.setHours(18,0,0,0);
    let cursor=selectedDay.toDateString()===new Date().toDateString()?roundQuarter(new Date(Math.max(Date.now(),start.getTime()))):start;
    const occupied=[...timed.map(event=>[new Date(event.startAt),new Date(event.endAt)] as const),...planned.map(item=>[new Date(item.plannedAt!),new Date(new Date(item.plannedAt!).getTime()+(item.plannedMinutes||estimate(item))*60000)] as const)].sort((a,b)=>a[0].getTime()-b[0].getTime());
    const available=(minutes:number)=>{ for(const [busyStart,busyEnd] of occupied){ if(cursor>=busyEnd)continue; if(cursor.getTime()+minutes*60000<=busyStart.getTime())break; if(cursor<busyEnd)cursor=roundQuarter(busyEnd); } if(cursor.getTime()+minutes*60000>end.getTime())return null; const slot=new Date(cursor); cursor=new Date(cursor.getTime()+minutes*60000); occupied.push([slot,new Date(cursor)]); occupied.sort((a,b)=>a[0].getTime()-b[0].getTime()); return slot; };
    return dayActions.filter(item=>!item.plannedAt).sort((a,b)=>(Date.parse(a.dueAt||"9999-12-31")-Date.parse(b.dueAt||"9999-12-31"))||(({urgent:0,high:1,normal:2,low:3}[a.priority as "urgent"|"high"|"normal"|"low"]??4)-({urgent:0,high:1,normal:2,low:3}[b.priority as "urgent"|"high"|"normal"|"low"]??4))).map(item=>({item,minutes:estimate(item),start:available(estimate(item))}));
  },[selectedDay,events,items]);

  const plan=async(item:Action,start:Date,minutes:number)=>{ setBusy(true); try { const response=await fetch(`${runnerUrl}/api/work-items/${encodeURIComponent(item.id)}`,{method:"PATCH",headers:{"content-type":"application/json"},body:JSON.stringify({plannedAt:start.toISOString(),plannedMinutes:minutes,eventDetail:`Planned for ${start.toLocaleString()}.`})}); const data=await response.json(); if(!response.ok)throw new Error(data.error||"The work block could not be saved."); onNotice(`Planned “${item.title}” for ${time(start.toISOString())}.`); onUpdated(); } catch(error){onNotice(error instanceof Error?error.message:"The work block could not be saved.");} finally{setBusy(false);} };

  const days=[0,1,2,3].map(offset=>addDays(dayStart(new Date()),offset));
  return <section className="calendar-view">
    <header className="calendar-heading"><div><p className="kicker">DAY AHEAD</p><h2>{selectedDay.toLocaleDateString([], {weekday:"long",month:"long",day:"numeric"})}</h2><p>Meetings from Outlook, with local work-block suggestions around them.</p></div><button disabled={busy} type="button" onClick={async()=>{setBusy(true);await fetch(`${runnerUrl}/api/calendar/refresh`,{method:"POST",headers:{"content-type":"application/json"},body:"{}"});window.setTimeout(()=>void load().finally(()=>setBusy(false)),1400);}}>{busy?"Refreshing...":"Refresh"}</button></header>
